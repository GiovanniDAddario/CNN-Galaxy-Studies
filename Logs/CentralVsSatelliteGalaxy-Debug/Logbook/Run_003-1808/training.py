...
t_label_data = train_label_data.tolist()
#print(t_label_data)
filename, label = t_label_data[0], t_label_data[1]
data = fits.getdata(train_data_dir+'/'+filename.decode("utf-8"))
train_data[0] = data
train_labels = np.array([int(label)])
...
#val_labels = np.zeros(shape=1)
# loop through file name/ label pairs: assing label to label array and data 
    # from file to 4D data array, guaranteeing that entries with the same index
    # in both arrays correspond to the same file
#for index, row in enumerate(val_label_data):
v_label_data = val_label_data.tolist()
filenamev, labelv = v_label_data[0], v_label_data[1]
data = fits.getdata(val_data_dir+'/'+filenamev.decode("utf-8"))
val_data[0] = data
val_labels = np.array([int(labelv)])
